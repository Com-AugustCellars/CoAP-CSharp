    ...
    public static class HttpTranslator
    {
        ...
        public static Response GetCoapResponse(HttpWebResponse httpResponse, Request coapRequest)
        {
            ...
            if (httpCode == HttpStatusCode.NoContent) {
                ...
                if (!http2coapCode.TryGetValue(httpCode, out coapCode)) throw ThrowHelper.TranslationException("Cannot convert the HTTP status " + httpCode);
            }
        ...
        public static Request GetCoapRequest(IHttpRequest httpRequest, String proxyResource, Boolean proxyingEnabled)
        {
            ...
            if (proxyResource == null) throw ThrowHelper.ArgumentNull("proxyResource");

            Method coapMethod;
            if (!http2coapMethod.TryGetValue(httpRequest.Method, out coapMethod)) throw ThrowHelper.TranslationException(httpRequest.Method + " method not mapped");
        ...
        public static IEnumerable<Option> GetCoapOptions(NameValueCollection headers)
        {
            ...
            foreach (String key in headers.AllKeys) {
                OptionType ot;
                if (!http2coapOption.TryGetValue(key, out ot)) continue;
        ...
        public static NameValueCollection GetHttpHeaders(IEnumerable<Option> optionList)
        {
            ...
            foreach (Option opt in optionList) {
                ...
                String headerName;
                if (!coap2httpHeader.TryGetValue(opt.Type, out headerName)) continue;
        ...
        public static void GetHttpResponse(IHttpRequest httpRequest, Response coapResponse, IHttpResponse httpResponse)
        {
            ...
            if (!coap2httpCode.TryGetValue(coapResponse.StatusCode, out httpCode)) throw ThrowHelper.TranslationException("Cannot convert the coap code in http status code: " + coapResponse.StatusCode);
            ...
            if (payload != null) {
                ...
                String contentType;
                if (coap2httpContentType.TryGetValue(coapResponse.ContentType, out contentType)) httpResponse.AppendHeader("content-type", contentType);
            }
        }
    }
}
