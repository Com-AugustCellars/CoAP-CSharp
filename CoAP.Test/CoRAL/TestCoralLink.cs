...
namespace CoAP.Test.Std10.CoRAL
{
    ...
    public class TestCoralLink
    {
        ...
        public void OneLink()
        {
            ...
            obj = link.EncodeToCBORObject(null, dictionary);
            Assert.AreEqual("[2, 99, 98]", obj.ToString());
        }
        ...
        public void ParseFromCBOR()
        {
            ...
            // [2, "relation", "value"]
            CoralLink link = new CoralLink(CBORObject.DecodeFromBytes(Hex.Decode("83026872656C6174696F6E6576616C7565")), ciri, testDictionary);
            ...
            // [2, 2, 4]
            link = new CoralLink(CBORObject.DecodeFromBytes(Hex.Decode("83020204")), ciri, testDictionary);
            ...
            // [2, 99, false]
            link = new CoralLink(CBORObject.DecodeFromBytes(Hex.Decode("83021863F4")), ciri, testDictionary);
            Assert.AreEqual(null, link.RelationType);
            Assert.AreEqual(99, link.RelationTypeInt);
            Assert.AreEqual(null, link.Body);

            // [2, 2, 2(999)]
            link = new CoralLink(CBORObject.DecodeFromBytes(Hex.Decode("830202C21903E7")), ciri, testDictionary);
            Assert.IsTrue(link.Value.IsTagged);
            Assert.AreEqual(1, link.Value.TagCount);
            Assert.AreEqual(2, link.Value.MostInnerTag);
            Assert.AreEqual(null, link.TargetInt);
            Assert.AreEqual(null, link.Body);

            // [1, 2, 4]
            Assert.ThrowsException<ArgumentException>(() => { link = new CoralLink(CBORObject.DecodeFromBytes(Hex.Decode("83010204")), ciri, testDictionary); }, "Not an encoded CoRAL link");

            // [2, true, false]
            Assert.ThrowsException<ArgumentException>(() => { link = new CoralLink(CBORObject.DecodeFromBytes(Hex.Decode("8302F5F4")), ciri, testDictionary); }, "Invalid relation in CoRAL link");

            // [2, 2, false, [2, 3, true]]
            link = new CoralLink(CBORObject.DecodeFromBytes(Hex.Decode("840202F481830203F5")), ciri, testDictionary);
            Assert.IsNotNull(link.Body);
            Assert.AreEqual(1, link.Body.Length);

            // [2, 2, 99999(5)]
            link = new CoralLink(CBORObject.DecodeFromBytes(Hex.Decode("830202DA0001869F05")), ciri, testDictionary);
            Assert.IsNotNull(link.Value.Type);
            Assert.AreEqual(CBORType.Integer, link.Value.Type);
            Assert.AreEqual(5, link.Value.AsInt32());

            // [ 2, 2, [5, 0]]
            link = new CoralLink(CBORObject.DecodeFromBytes(Hex.Decode("830202820500")), ciri, testDictionary);
            Assert.IsNotNull(link.Target);
            Assert.AreEqual(ciri.Data.ToString(), link.Target.Data.ToString());

            // [ 2, 2, [5, 0, 6, "path2"], [[ 2, 2, [5, 0, 6, "path3"], [5, 2, 6, "path4"]]]]
            link = new CoralLink(CBORObject.DecodeFromBytes(Hex.Decode("84020284050006657061746832828302028405000665706174683383020284050206657061746834")), ciri, testDictionary);
            Assert.IsNotNull(link.Target);
        ...
        public void Constructors()
        {
            ...
            link = new CoralLink("relation", new Ciri("coap://host:5/root/path"));
            Assert.AreEqual("relation", link.RelationType);
            ...
            // Assert.ThrowsException<ArgumentException>(() => link = new CoralLink("relation", new Ciri(CBORObject.DecodeFromBytes(Hex.Decode("8102")))));
        }
        ...
        public void TestEncode()
        {
            ...
            Assert.AreEqual("[2, 0, 2]", cbor.ToString());
            ...
            Assert.IsTrue(cbor[2].HasOneTag(99999));
            Assert.AreEqual(15, cbor[2].Untag().AsInt32());
            Assert.AreEqual("[2, \"relation\", 99999(15)]", cbor.ToString());

            link = new CoralLink("http://www.w3.org/1999/02/22-rdf-syntax-ns#type", new Ciri(CBORObject.DecodeFromBytes(Hex.Decode("820501"))));
            cbor = link.EncodeToCBORObject(null, testDictionary);
